...
function SummaryView({
  state,
  sendCommand,
}: ViewProps<SrtOutputSettings, SrtOutputState, SrtOutputCommand>) {
        ...
        <div className="flex items-center space-x-2">
          <span
            className={`inline-block w-2 h-2 rounded-full ${
              state.enabled ? "bg-green-500" : "bg-red-500"
            }`}
          ></span>
        ...
        <div>
          {state.enabled ? (
            ...
            <button
              ...
              className="px-3 py-1 text-sm bg-blue-600 hover:bg-green-700 text-white rounded"
            >
              Enable Output
            </button>
          )}
