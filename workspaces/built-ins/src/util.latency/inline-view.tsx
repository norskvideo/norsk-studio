...
import type { LatencyStatsOutputState, LatencyStatsOutputSettings } from "./runtime";

import Chart from 'chart.js/auto';

function InlineView({ state, config: _2 }: { state: LatencyStatsOutputState, config: LatencyStatsOutputSettings }) {
  ...
  function makeData(state: LatencyStatsOutputState) {
    return {
      labels: new Array(state.values.length).fill(0).map((_, i) => i).map((_) => ""),
      datasets: [makeDataSet("latency", 'rgba(255, 0, 0, 255)', state.values)]
    }
  }
